...
namespace MovieDatabase
{
    ...
    public class UserInterface
    {
        ...
        public UserInterface()
        {
            ...
            ProgramHeader1 = "This program provide you further information " +
                "on thousands of movies.";
            ProgramHeader2 = "Data souce: Public database available on IMDb " +
        ...
        private string GetUserChoice(string validLetters = "", 
            bool allowNumbers = false, Int16 minNumber = 0, 
            Int16 maxNumber = 999)
        {
            ...
            do
            {
                ...
                if (Char.IsLetterOrDigit(response.KeyChar))
                {
                    ...
                    {
                        if (Int16.TryParse(result + response.KeyChar, 
                            out Int16 numberResponse))
                        {
                            if ((numberResponse >= minNumber || 
                                numberResponse.ToString().Length < 
                                minNumber.ToString().Length)
                    ...
                    {
                        if (validLetters == "" || validLetters.ToLower()
                            .Contains(Char.ToLower(response.KeyChar)))
                        {
        ...
        public string SearchFor(string sectionTitle)
        {
            ...
            do
            {
                Header();
                Console.WriteLine();
                Console.WriteLine("     Please enter with the " +
                    "" + sectionTitle + " name for searching");
                Console.WriteLine("or press ENTER to return. You " +
                    "can type only a part of the name:");
                option = Console.ReadLine();
                if (option == "")
                {
                    Console.WriteLine("Do you confirm return to main menu? " +
                        "(Y/N):");
        ...
        public string GetSelectedTitle(Query moviesDB, string criteria = "")
        {
            ...
            do
            {
                ...
                restart = false;
                // Loop that goes through the list of titles that contains 
                //the word/s that the user typed
                        ...
                        if (i > 19) // end of page
                        {
                            ...
                            if (currentPage == 1)
                            {
                                Console.WriteLine("Please enter the " +
                                    "selected ITEM or [Q] to quit, [N] to " +
                                    "next page");
                                option = GetUserChoice("qn", true, minItem, 
                                    maxItem);
                            }
                            else
                            {
                                Console.WriteLine("Please enter the " +
                                    "selected ITEM or [Q] to quit, [N] to" +
                                    " next page, [P] to previous page:");
                                option = GetUserChoice("qnp", true, 
                                    minItem, maxItem);
                            }

                            // User choose or quit
                            if ((Byte.TryParse(option, out byte bOp)) || 
                                (option == "q"))
                                break;
                ...
                if (!morePages)
                {
                    Console.WriteLine("-----------------------------END-----" +
                        "-------------------------------");
                    Console.WriteLine("Please enter the selected ITEM or" +
                        " [Q] to quit, [P] to previus page");
        ...
        public string GetSelectedPersons(Query moviesDB, string criteria = "")
        {
            ...
            do
            {
                ...
                Header();
                Console.WriteLine("Filter Criteria: " + criteria);
                Console.WriteLine("ITEMS\t| Primary Name\t| " +
                    "Primary Professions\n");
                restart = false;
                // Loop that goes through the list of persons that contains the
                //word/s that the user typed
                foreach (Person field in moviesDB.FilteredNames)
                {
                    if (currentPage == selectedPage)
                    {
                        ...
                        minItem = (minItem == 255) ? Byte.Parse(field.ID) : 
                            minItem;
                        maxItem = Byte.Parse(field.ID);
                        i++;
                        if (i > 19) // end of page
        ...
        public string TitleDetails(Query moviesDB)
        {
            ...
            foreach (Details field in moviesDB.FilteredDetails)
            {
                ...
                Console.WriteLine("{0,17}: {1}", "Original Name", 
                    field.OriginalTitle);
                Console.WriteLine("{0,17}: {1}", "Title Type", 
                    field.TitleType);
                Console.WriteLine("{0,17}: {1}", "Genre", field.Genres);
                Console.WriteLine("{0,17}: {1}", "Average Rating", 
                    field.AverageRating);
                Console.WriteLine("{0,17}: {1}", "Number of Votes", 
                    field.NumVotes);
                Console.WriteLine("{0,17}: {1}", "Start Year", 
                    field.StartYear);
                Console.WriteLine("{0,17}: {1}", "End Year", field.EndYear);
                Console.WriteLine("{0,17}: {1}", "Duration", 
                    field.RuntimeMinutes);
                Console.WriteLine("{0,17}: {1}", "Adults Only", field.IsAdult);
                if (field.TitleType == "tvEpisode")
                {
                    Console.WriteLine("{0,17}: {1}", "From the series", 
                        field.ParentTitle);
                    Console.WriteLine("{0,17}: {1}", "Season", 
                        field.SeasonNumber);
                    Console.WriteLine("{0,17}: {1}", "Episode", 
                        field.EpisodeNumber);
                    hasParent = true;
                }
        ...
        public string PersonDetails(Query moviesDB)
        {
            ...
            foreach (Person field in moviesDB.FilteredNameDetails)
            {
                ...
                Console.WriteLine("{0,17}: {1}", "Birth Year", 
                    field.BirthYear);
                Console.WriteLine("{0,17}: {1}", "Death Year", 
                    field.DeathYear);
                Console.WriteLine("{0,17}: {1}", "Primary professions", 
                    field.PrimaryProfession);
            }
